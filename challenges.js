...
const poll = {
   ...
   registerNewAnswer(){
    ...
    /*
    Main components of the poll app: 
    Ask a question,-> prompt
    receive answer-> store the result
    update answers array properly depending on position
    check if the input is a number and inpit makes sense with the options provided
    call the method when the user clicks answer
    New method to display the poll results : "The
    method takes a string as an input (called 'type'), which can be either 'string'
    or 'array'. If type is 'array', simply display the results array as it is, using
    console.log(). This should be the default option. If type is 'string', display a
    string like "Poll results are 13, 2, 4, 1"."
